...
public class SocialMediaController {
    ...
    public ResponseEntity<?>registerUser(@RequestBody Account account){
        Optional<Account> existUser = accountService.findByUsername(account.getUsername());
    ...
    public ResponseEntity<?> loginUser(@RequestBody Account account){
        Optional<Account> authAcc = accountService.authenticateUser(account.getUsername(), account.getPassword());
            return authAcc.map(ResponseEntity::ok).orElseGet(() ->  ResponseEntity.status(401).build());
    }
    ...
    public ResponseEntity<?> getMessageById(@PathVariable Integer messageId) {
        ...
        return message.isPresent() ? ResponseEntity.ok(message.get()) : ResponseEntity.ok().body("");  
    }
